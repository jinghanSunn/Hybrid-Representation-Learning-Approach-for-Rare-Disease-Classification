...
import moco.loader
import moco.builder
from dataset import ISICDataset, ISICTestDataset, ISICTestODataset, ISICPretrainDatasetfoTrain, ISICTestODataset, ISICPretrainDatasetfoIncreTrain, ISICTestOIncreDataset
from dataset_papsmear import PAPSmearTestODataset, PAPSmearPretrainDatasetfoTrain
from dataset_oct import OCTTestODataset, OCTPretrainDatasetfoTrain
...
from model.resnet import resnet12, resnet18, resnet50
...
parser.add_argument('--increment', action='store_true', help='increment testing')
...
def main_worker(args):
    ...
    if args.dataset == 'isic':
        if args.increment:
            pretrain_dataset = ISICPretrainDatasetfoIncreTrain(root=testdir, task_num=args.task_num, n_way=args.n_way, k_shot=args.k_shot, k_query=args.k_query)
        else:
        ...
        if args.test == 'normal' or args.test == 'hard':
            test_dataset = ISICTestDataset(root=testdir, task_num=args.task_num, n_way=args.n_way, k_shot=args.k_shot, k_query=args.k_query, selected_cls=args.select_cls, test=args.test)
        elif args.test == 'all':
            if args.increment:
                test_dataset = ISICTestOIncreDataset(root=testdir, task_num=args.task_num, n_way=args.n_way, k_shot=args.k_shot, k_query=args.k_query, selected_cls=args.select_cls)
            else:
                test_dataset = ISICTestODataset(root=testdir, task_num=args.task_num, n_way=args.n_way, k_shot=args.k_shot, k_query=args.k_query, selected_cls=args.select_cls)
        else:
        ...
        pretrain_dataset = OCTPretrainDatasetfoTrain(root=testdir, task_num=args.task_num, n_way=args.n_way, k_shot=args.k_shot, k_query=args.k_query)
        test_dataset = OCTTestODataset(root=testdir, task_num=args.task_num, n_way=args.n_way, k_shot=args.k_shot)
    test_sampler = None
    ...
    for i in range(args.test_time):
        ...
        # if args.arch[:6] == 'resnet':
            ...
            # print("train LG after average pooling")
        # print(extractor)
        ...
        if args.pretrain == 'once':
            for x, y in pretrain_loader:
                ...
                clf = LogisticRegression(random_state=0, max_iter=200).fit(normalize(extractor(x.squeeze()).squeeze().detach().cpu()), y.squeeze())
            print("fit once")
        ...
        #     plot_acc_loss(x, ALL_corrects, path=args.visual_dir, name='accuracy', save_name = args.resume.split('/')[-2]+ f'shot{args.k_shot}' +'_accuracy')
            ...
            for data in tqdm(test_loader):
                ...
                with torch.no_grad():
                    if args.pretrain == 'normal':
                        clf = LogisticRegression(random_state=0, max_iter=200).fit(extractor(x_spt.squeeze()).squeeze().detach().cpu(), y_spt.squeeze())
                    # q_feature = extractor(x_qry).detach().cpu()
                    q_feature = normalize(extractor(x_qry).detach()).squeeze().cpu()
                    pred_q = clf.predict(q_feature.reshape(1,-1))
                    prob_q = clf.predict_proba(q_feature.reshape(1,-1))
                    pred.extend(pred_q)
                    prob.extend(prob_q)
                    true.extend(y_qry)
                    # print(pred_q)
            ...
            print("accuracy:", np.mean(corrects)*100)
            # print("std:", std)
            print("confidence interval:",conf_intveral)
            print("AUC_sample:", np.mean(AUC_sample))
            print("AUC_micro:", np.mean(AUC_micro))
            ...
            if args.dataset == 'isic':
                ...
                acc_1 = (torch.eq(torch.Tensor(pred)[:index1], torch.Tensor(true)[:index1]).sum().item())/float(index1)
                acc_2 = (torch.eq(torch.Tensor(pred)[index1:index1+index2], torch.Tensor(true)[index1:index1+index2]).sum().item())/float(index2)
                acc_3 = (torch.eq(torch.Tensor(pred)[index1+index2:], torch.Tensor(true)[index1+index2:]).sum().item())/float(index3)
                print(acc_1*100)
                print(acc_2*100)
                print(acc_3*100)
                print("avg:", (acc_1+acc_2+acc_3)/3*100)
                ...
                index1 = 32 - args.k_shot
                index2 = 29 - args.k_shot
                index3 = 31 - args.k_shot
                index4 = 31 - args.k_shot
                index5 = 23 - args.k_shot
                print(index1,index2,index3)
                acc_1 = (torch.eq(torch.Tensor(pred)[:index1], torch.Tensor(true)[:index1]).sum().item())/float(index1)
                acc_2 = (torch.eq(torch.Tensor(pred)[index1:index1+index2], torch.Tensor(true)[index1:index1+index2]).sum().item())/float(index2)
                acc_3 = (torch.eq(torch.Tensor(pred)[index1+index2:index1+index2+index3], torch.Tensor(true)[index1+index2:index1+index2+index3]).sum().item())/float(index3)
                acc_4 = (torch.eq(torch.Tensor(pred)[index1+index2+index3:index1+index2+index3+index4], torch.Tensor(true)[index1+index2+index3:index1+index2+index3+index4]).sum().item())/float(index4)
                acc_5 = (torch.eq(torch.Tensor(pred)[index1+index2+index3+index4:], torch.Tensor(true)[index1+index2+index3+index4:]).sum().item())/float(index5)
                print(acc_1*100)
                print(acc_2*100)
                print(acc_3*100)
                print(acc_4*100)
                print(acc_5*100)
                print("avg:", (acc_1+acc_2+acc_3+acc_4+acc_5)/5*100)
            # 32,29,31,31,23
        
            np.save(f'./ISIC/Pred/ISIC_MoCo_shot{args.k_shot}.npy', np.array(pred))
            print('save to', f'./ISIC/Pred/ISIC_MoCo_shot{args.k_shot}.npy')
